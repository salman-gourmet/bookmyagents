                    ...
                    <div key={result.id} className="col-xxl-4 col-xl-6 col-lg-6 col-md-6 tg-grid-full">
                        <div className="tg-listing-card-item mb-30">
                            <div className="tg-listing-card-thumb fix mb-15 p-relative">
                                <Link to={`/tour-details?id=${result.service?._id || result.id}`}>
                                    <img
                                        className="tg-card-border w-100"
                                        src={
                                            result.service?.pictures && result.service.pictures.length > 0
                                                ? result.service.pictures[0]
                                                : result.service?.tour?.images && result.service.tour.images.length > 0
                                                    ? result.service.tour.images[0]
                                                    : result.imageUrls && result.imageUrls.length > 0
                                                        ? result.imageUrls[0]
                                                        : "/assets/img/placeholder/placeholder.png"
                                        }
                                        alt={result.service?.title || result.service?.tour?.title || result.name}
                                    />
                                    {result.service?.isAd || result.isAd ? (
                            ...
                            <div className="tg-listing-main-content">
                                <div className="tg-listing-card-content">
                                    <h4 className="tg-listing-card-title">
                                        <Link to={`/tour-details?id=${result.service?._id || result.id}`}>
                                            {result.service?.title || result.service?.tour?.title || result.name}
                                        </Link>
                                    </h4>
                                    <div className="tg-listing-card-duration-tour">
                                        <span className="tg-listing-card-duration-map mb-5">
                                            ...
                                            {result.service?.tour?.location || result.service?.location?.city || result.address}
                                        </span>
                                        {result.service?.bookingOptions?.duration && (
                                ...
                                <div className="tg-listing-card-price d-flex align-items-end justify-content-between">
                                    <div className="tg-listing-card-price-wrap price-bg d-flex align-items-center">
                                        <span className="tg-listing-card-currency-amount mr-5">
                                            <span className="currency-symbol">
                                                {result.service?.tour?.currency === 'EUR' ? '€' :
                                                    result.service?.tour?.currency === 'GBP' ? '£' : '$'}
                                            </span>
                                            {result.service?.tour?.price || result.service?.price || 99}
                                        </span>
                                    ...
                                    <div className="tg-listing-card-review space">
                                        ...
                                        {result.service?.tour?.rating && (
                                            <span className="tg-listing-rating-percent">
                                                {result.service.tour.rating.toFixed(1)} ({result.service.tour.numberOfReviews || 0} Reviews)
                                            </span>
                                        )}
                                        {!result.service?.tour?.rating && (
                                            <span className="tg-listing-rating-percent">
                                                ({result.service?.tour?.numberOfReviews || 0} Reviews)
                                            </span>
                                        )}
                                    </div>
                                </div>
