...
const BlogArea = () => {
   ...
   const fetchBlogs = async () => {
      try {
         setLoading(true);
         setError(null);
         const response = await blogService.getPublishedBlogs(currentPage, itemsPerPage);
                              ...
                              <div className="tg-blog-grid-item mb-30">
                                 ...
                                 <div className="tg-blog-standard-content">
                                    <h2 className="tg-blog-standard-title">
                                       <Link to={`/blog-details?id=${blog._id}`}>{blog.title}</Link>
                                    </h2>
                                    <div className="tg-blog-standard-date mb-10">
                                       ...
                                       <span>
                                          ...
                                          {new Date(blog.publishedAt || blog.createdAt).toLocaleTimeString('en-US', {
                                             hour: '2-digit',
                                             minute: '2-digit'
                                          })}
                                       </span>
                                    </div>
                                    <div className="mb-20 tg-blog-standard-para" dangerouslySetInnerHTML={{
                                       __html: blog.content.length > 150
                                          ? `${blog.content.substring(0, 150)}...`
                                          : blog.content
                                    }} />
                                    <div className="tg-blog-sidebar-btn d-flex justify-content-between align-items-center">
                                       <Link to={`/blog-details?id=${blog._id}`} className="tg-btn tg-btn-switch-animation">
